...
# start sumo 
'''
If you want us sumo-gui,  sumoCmd should be changed to " sumoCmd= ["sumo-gui", "-c", "osm.sumocfg"] " 
'''
...
while traci.simulation.getMinExpectedNumber() > 0:
    ...
    vehicles = traci.vehicle.getIDList();  # get all vehicles
    trafficlights = traci.trafficlight.getIDList();  # get all trafficlights

    # 1-1. first , deal with the vehicles's data
    for i in range(0, len(vehicles)):

        # Function descriptions
        # https://sumo.dlr.de/docs/TraCI/Vehicle_Value_Retrieval.html
        # https://sumo.dlr.de/pydoc/traci._vehicle.html#VehicleDomain-getSpeed
        vehid = vehicles[i]  # vehicle id
        x, y = traci.vehicle.getPosition(vehicles[i])  # GPS position
        coord = [x, y]
        lon, lat = traci.simulation.convertGeo(x, y)  # (lon,lat)
        gpscoord = [lon, lat]
        spd = round(traci.vehicle.getSpeed(vehicles[i]) * 3.6, 2)  # speed
        ...
        # 1-2.then, check wether the road is contorlled by trafficlights
        for k in range(0, len(trafficlights)):
            ...
            if idd in traci.trafficlight.getControlledLanes(trafficlights[k]):
                # if it is , get trafficlights's information
        ...
        NEWSPEED = 15  # speed value in m/s (15 m/s = 54 km/hr)
        if vehicles[i] == 'veh2':
